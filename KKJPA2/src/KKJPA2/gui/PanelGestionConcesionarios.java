...
public class PanelGestionConcesionarios extends JPanel {
		...
		this.addMouseWheelListener(new MouseWheelListener() {
			...
			public void mouseWheelMoved(MouseWheelEvent e) {
				if (e.getUnitsToScroll() < 0) {
					if (ControladorBBDDConcesionario.findNext(concesionario) != null) {
						navegaASiguiente();
					}
				}
				else {
					if (ControladorBBDDConcesionario.findPrevious(concesionario) != null) {
						navegaAAnterior();
					}
				}
			}
	...
	private void eliminar() {
		...
		if (ControladorBBDDConcesionario.findLast().getId() == this.concesionario.getId()) {
			navegaAAnterior();
		}
		else {
			navegaASiguiente();
		...
		ControladorBBDDConcesionario.remove(concesionarioAEliminar);
	...
	private void guardar() {
		// Es un alta nueva o una modificaci�n
				...
				this.concesionario = ControladorBBDDConcesionario.persist(this.concesionario);
				cargaConcesionarioEnComponentesVisuales();
				if (!ControladorBBDDConcesionario.exist(this.concesionario)) {
					this.navegaAUltimo();
				}
	...
	private void navegaAPrimero () {
		concesionario = ControladorBBDDConcesionario.findFirst();
		cargaConcesionarioEnComponentesVisuales();
	...
	private void navegaAUltimo () {
		concesionario = ControladorBBDDConcesionario.findLast();
		cargaConcesionarioEnComponentesVisuales();
		actualizaEstadoBotonera();
	}
	...
	private void navegaASiguiente () {
		concesionario = ControladorBBDDConcesionario.findNext(this.concesionario);
		cargaConcesionarioEnComponentesVisuales();
		actualizaEstadoBotonera();
	}
	...
	private void navegaAAnterior () {
		concesionario = ControladorBBDDConcesionario.findPrevious(this.concesionario);
		cargaConcesionarioEnComponentesVisuales();
		actualizaEstadoBotonera();
	}
	...
	private void actualizaEstadoBotonera () {
		if (ControladorBBDDConcesionario.findPrevious(this.concesionario) == null) {
		...
		if (ControladorBBDDConcesionario.findNext(this.concesionario) == null) {
